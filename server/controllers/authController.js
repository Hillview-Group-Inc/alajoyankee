    ...
    // PRD §6.1 — fire-and-forget welcome notification
    {
      ...
      const tplData = {
        ...
        rows: [
          ...
          ['Member since',   new Date(newUser.CreatedAt).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })],
          ['Account type',   `<span style="color:#1a6b3c;font-weight:600;">Member</span>`],
        ],
        ctaLabel: 'Open your dashboard',
        ctaUrl:   `${baseUrl}/dashboard.html`,
        closing:  'Questions? Reply to this email — our community team is here to help.',
      };
      notifyUser({
        user:    { userID: newUser.UserID, email: newUser.Email, phone: newUser.Phone },
        subject: 'Welcome to Alajo Yankee 🎉',
        message: renderText(tplData),
        html:    renderEmail(tplData),
      }).catch(err => console.warn('Welcome notification failed:', err.message));
    }
...

    {
      const tplData = {
        ...
        rows: [
          ...
          ['Link expires in', `<strong>${RESET_TOKEN_TTL_MIN} minutes</strong>`],
          ['Single-use',  `Yes — the link works once and is invalidated immediately after use.`],
        ],
